...
SCALA_VERSION=${VERSION:-"none"}
SBT_VERSION=${SBT:-"none"}
...
sdk_install() {
    ...
    if [ "${set_as_default}" = "true" ]; then
        SCALA_VERSION=${requested_version}
    fi
...
sdk_install scala ${SCALA_VERSION} "\\s*" ".*-[a-z]+$" "true"
sdk_install sbt ${SBT_VERSION} "\\s*" ".*-[a-z]+$" "true"
